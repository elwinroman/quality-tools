...
# Uso:
#   bash setup/02_build_and_deploy.sh           → usa APP_VERSION de .env.compose
...
ENV_FILES="--env-file .env.compose --env-file .env.valkey"
...
echo ">>> Deploy completado. Versión: ${APP_VERSION:-$(grep APP_VERSION .env.compose | cut -d= -f2)}"
